...
def delete_old_hash_browns(output_index: pd.Index, path: Path):
    ...
    hash_brown_files = [
        get_full_hash_path(path, f)
        for f in listdir(path / "Hashbrowns")
        if Path(f).suffix == ".hash_brown"
    ]
    delete_hash_paths = [
        f for f in hash_brown_files if f.stem + f.suffix not in output_index
    ]
...
def try_decrypting_dict(
    note_data: dict, file_name: Path, encrypt_fields: set, eval_fields: set
):
    """Attempt to decrypt dictionary mapping to encrypted data"""
    try:
        decrypted_note = decrypt_json_dict(
            note_data, file_name, encrypt_fields, eval_fields
        )
...
def load_content_from_csv(
    content_path: Path, encrypt_fields: List[str], eval_fields: List[str]
) -> Dict[str, dict]:
    ...
    for file_name, note_data in loaded_dicts.items():
        ...
        if decrypted_note := try_decrypting_dict(
            note_data, file_path, encrypt_fields, eval_fields
        ):
...
def save_model_data(
    ...
    save_df = create_updated_df(
        original_df, new_data, original_data, save_path.parent, encrypt_fields
    )
