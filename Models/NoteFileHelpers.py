...
def delete_old_hash_browns(output_df: pd.DataFrame, path: Path):
    hash_brown_files = [path/f for f in listdir(path) if Path(f).suffix == '.hash_brown']
...
def load_notes_from_folder(path: Path, encrypt_fields: set, eval_fields: set):
    ...
    decrypted_notes = {}
    for file_name, json_note in loaded_dicts.items():
        file_path = path/file_name
        if decrypted_note := try_decrypting_note(json_note, file_path, encrypt_fields, eval_fields):
