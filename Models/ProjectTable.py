...
from UI.DisplayParameters import (
    PROJECT_TABLE_LEFT_ALGIN_COLUMNS,
...
class ProjectTableModel(QAbstractTableModel):
    ...
    def __init__(self, data: Optional[List[Project]] = None):
        ...
        self._df = pd.DataFrame(
            display_data, columns=PROJECT_FIELDS_TO_DISPLAY.values()
        )
    ...
    def data(
        self, ind: QModelIndex, role=Qt.DisplayRole
    ) -> Union[Qt.AlignmentFlag, str, None]:
    ...
    def setData(self, ind: QModelIndex, value: str, role: int) -> bool:
        ...
        if (
            role == Qt.EditRole
            and self._df.iloc[ind.row(), ind.column() - 1] != value
            and ind.isValid()
        ):
    ...
    def headerData(
        self, col: int, orientation: Qt.Orientation, role: int
    ) -> Optional[str]:
    ...
    def _update_data_with_id(
        self, project_id: int, **kwargs
    ) -> Optional[Tuple[int, Project]]:
