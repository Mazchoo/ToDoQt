...
from Controller.ControlHelpers import (
    ...
    enable_time_edits,
    disable_time_edits,
    clear_new_task_entry,
    enable_task_controls,
)
...
def add_new_task_to_pending(self: ToDoListController, _click: bool):
    ...
    if (task_name := self.layout.newTask_lineEdit.text()) and (
        ...
        append_item_to_list_view(
            self.model.pending_list,
            self.model.pending_filter,
...
def select_current_task(
    self: ToDoListController, selected_task: Optional[QStandardItem]
):
...
@ClassMethod(ToDoListController)
@QtControlFunction()
def save_current_task_description(self: ToDoListController):
    ...
    if selected_task := get_selected_task(self.model, self.layout):
        ...
        if description != old_description:
            ...
            update_fields = {
                "description": selected_task.accessibleDescription(),
