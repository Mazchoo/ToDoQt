...
class ToDoListController(QWidget):
    ...
    def setupCallbacks(controller):
        ...
        controller.layout.deleteTask_pushButton.clicked.connect(
            lambda x: controller.delete_current_task(x)
        )
        controller.layout.close_pushButton.clicked.connect(
            lambda x: controller.close_window(x)
        )

        pending_list.clicked.connect(lambda x: controller.setFocus_to_pendingView(x))
        in_progress_list.clicked.connect(
            lambda x: controller.setFocus_to_in_progressView(x)
        )
        done_list.clicked.connect(lambda x: controller.setFocus_to_doneView(x))

        controller.layout.backup_pushButton.clicked.connect(
            lambda x: controller.save_backups(x)
        )
        controller.layout.upload_pushButton.clicked.connect(
            lambda x: controller.git_push_backups(x)
        )

        setattr(
            pending_list,
            "dragEnterEvent",
            lambda e: enter_task_list_box(controller.layout, pending_list, e),
        )
        setattr(
            pending_list, "dragMoveEvent", lambda e: drag_move_event(pending_list, e)
        )
        setattr(
            pending_list,
            "dropEvent",
            lambda e: move_task_list_item(controller, pending_list, e),
        )

        setattr(
            in_progress_list,
            "dragEnterEvent",
            lambda e: enter_task_list_box(controller.layout, in_progress_list, e),
        )
        setattr(
            in_progress_list,
            "dragMoveEvent",
            lambda e: drag_move_event(in_progress_list, e),
        )
        setattr(
            in_progress_list,
            "dropEvent",
            lambda e: move_task_list_item(controller, in_progress_list, e),
        )

        setattr(
            done_list,
            "dragEnterEvent",
            lambda e: enter_task_list_box(controller.layout, done_list, e),
        )
        setattr(done_list, "dragMoveEvent", lambda e: drag_move_event(done_list, e))
        setattr(
            done_list,
            "dropEvent",
            lambda e: move_task_list_item(controller, done_list, e),
        )

        controller.layout.addNewTask_pushButton.clicked.connect(
            lambda x: controller.add_new_task_to_pending(x)
        )
        controller.layout.newTask_lineEdit.textChanged.connect(
            lambda: controller.enable_add_new_task()
        )

        controller.layout.addNewProject_pushButton.clicked.connect(
            lambda x: controller.add_new_project(x)
        )
        controller.layout.newProject_lineEdit.textChanged.connect(
            lambda: controller.enable_add_new_project()
        )
        controller.layout.deleteProject_pushButton.clicked.connect(
            lambda x: controller.delete_current_project(x)
        )
        controller.layout.archiveProject_pushButton.clicked.connect(
            lambda x: controller.archive_current_project(x)
        )
        ...
        controller.layout.estimatedTime_timeEdit.timeChanged.connect(
            lambda x: controller.edit_time_estimate_spinner(x)
        )
        controller.layout.points_spinBox.valueChanged.connect(
            lambda x: controller.edit_points_spinner(x)
        )

        controller.model.pending_list.dataChanged.connect(
            lambda: controller.task_title_changed()
        )
        controller.model.in_progress_list.dataChanged.connect(
            lambda: controller.task_title_changed()
        )
        controller.model.done_list.dataChanged.connect(
            lambda: controller.task_title_changed()
        )

        controller.task_description_handler = MarkdownFocusHandler(
            controller.layout.taskDescription_textEdit
        )
        controller.task_description_handler.focusOut.connect(
            lambda: controller.save_current_task_description()
        )

        controller.project_description_handler = MarkdownFocusHandler(
            controller.layout.projectDescription_textEdit
        )
        controller.project_description_handler.focusOut.connect(
            lambda: controller.save_project_description()
        )

        controller.timer = TimerEvents(controller.layout.timeSpent_timeEdit)
        controller.layout.recordingTime_pushButton.clicked.connect(
            lambda x: controller.toggle_record_time(x)
        )

        controller.layout.project_tableView.clicked.connect(
            lambda x: controller.project_row_click(x)
        )
        controller.layout.project_tableView.horizontalHeader().sectionClicked.connect(
            lambda x: controller.project_header_click(x)
        )
        controller.model.project_list.dataUpdated.connect(
            lambda: controller.enable_project_save()
        )
    ...
    def initializeModels(controller):
        ...
        # Update level display based on total points
        update_level_display(controller.layout, total_points_all_projects)
