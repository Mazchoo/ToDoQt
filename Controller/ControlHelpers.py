...
def get_selected_item_from_list(model_list: QStandardItemModel, model_filter: ProjectFilterProxyModel,
                                list_view: QListView) -> Optional[QStandardItem]:
...
def get_selected_task(model: ToDoModel, layout: Ui_ToDoLayout):
    ...
    if selected_item := get_selected_item_from_list(model.in_progress_list, model.in_progress_filter,
                                                    layout.inProgress_listView):
        return selected_item
...
def delete_selected_task(model: ToDoModel, layout: Ui_ToDoLayout):
    ...
    if deleted_item := delete_item_if_selected(model.pending_list, model.pending_filter, layout.pending_listView):
        return deleted_item
    if deleted_item := delete_item_if_selected(model.in_progress_list, model.in_progress_filter, layout.inProgress_listView):
        return deleted_item
    if deleted_item := delete_item_if_selected(model.done_list, model.done_filter, layout.done_listView):
        return deleted_item
    return None


def delete_tasks_with_project_id(model_list: QStandardItemModel, model_filter: ProjectFilterProxyModel,
                                 project_id: int):
...
def delete_all_tasks_with_project_id(model: ToDoModel, project_id: int):
    ...
    delete_tasks_with_project_id(model.in_progress_list, model.in_progress_filter, project_id)
    delete_tasks_with_project_id(model.done_list, model.done_filter, project_id)


def get_corresponding_model(model: ToDoModel, layout: Ui_ToDoLayout, list_view: QListView) -> \
...
def filter_available_tasks_for_selected_project(model: ToDoModel, project_id: Optional[int]):
...
def enable_time_edits(self: Self, selected_task: QStandardItem):
    ...
    with block_signals(self.layout.timeSpent_timeEdit) as time_spent_spinner:
        time_spent_spinner.setTime(get_qt_time_from_seconds(selected_task.data()['time_spent_seconds']))
    with block_signals(self.layout.estimatedTime_timeEdit) as estimate_spinner:
        estimate_spinner.setTime(get_qt_time_from_seconds(selected_task.data()['estimated_time_seconds']))
...
def get_default_hour_to_points_valuation(self: Self, task: QStandardItem, total_seconds: int) -> int:
