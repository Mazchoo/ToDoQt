...
def parse_field(decrpyted_value: str, key: str, eval_fields: set):
    ...
    output = (
        ast.literal_eval(decrpyted_value) if key in eval_fields else decrpyted_value
    )
...
def decrypt_json_dict(
    field_value_pairs: dict,
    key_file_name: Path,
    decrypt_fields: Set[str],
    eval_fields: Set[str],
):
    """
    ...
    for key, value in field_value_pairs.items():
        decrpyted_value = (
            fernet_decrypt_string(fernet, value) if key in decrypt_fields else value
        )
